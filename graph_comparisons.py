...
def plot_log_roc_curves(metrics_list: List[Tuple[np.ndarray, np.ndarray, float]], 
    ...
    plt.ylim([1.0, 1.0e6])
    plt.yscale('log')
    plt.xlabel('Signal Efficiency (TPR)', fontsize=19)
    plt.ylabel('Background Rejection (FPR$^{-1}$)', fontsize=19)
    plt.title('Jet Classification Performance - Log ROC', fontsize=21, fontweight='bold')
