...
Date: ante diem VII Kalendas Octobres anno ab urbe condita MMDCCLXXVIII

Test script for QFI-based Jet GNN classification with attention matrix analysis.
Loads trained model, evaluates on test data, and visualizes QFI matrices and QFI-derived attention matrices by jet class.
"""
import argparse
import sys
import os
...
class QFIAttentionPlotter:
    """
    QFI matrix and attention matrix plotting utility for visualization and class-separated comparison.
    """
    
    def __init__(self, output_dir: str):
        """
        Initialize QFI attention plotter.
        ...
        logger.info(f"QFI attention plotter initialized, saving to: {output_dir}")
    ...
    def plot_qfi_matrix(self, qfi_matrix: np.ndarray, plot_label: str, 
        ...
        # Print statistics
        if show_stats:
            logger.info(f"Matrix Statistics for {plot_label}:")
    ...
    def plot_attention_matrix(self, attention_matrix: np.ndarray, plot_label: str, 
                             save_name: str, show_stats: bool = True) -> None:
        """
        Plot attention matrix with quantum circuit style formatting and attention-specific colormap.
        
        Args:
            attention_matrix: Array of shape (30, 30) - Attention matrix
            plot_label: Label for the plot title
            save_name: Filename for saving (without extension)
            show_stats: Whether to print matrix statistics
        """
        ...
        # Get matrix dimensions
        N_params = attention_matrix.shape[0]  # Should be 30
        N_qubits = N_params // 3        # Should be 10
        
        # Create figure
        fig, ax = plt.subplots(1, 1, figsize=(12, 10))
        
        # Attention-specific color scheme (white to dark blue)
        cmap = plt.cm.Blues
        
        # Set up normalization (attention weights are typically positive)
        vmin = 0.0
        ...
        # Print statistics
        if show_stats:
            logger.info(f"Attention Matrix Statistics for {plot_label}:")
            logger.info(f"  Range: [{attention_matrix.min():.6f}, {attention_matrix.max():.6f}]")
            logger.info(f"  Mean: {attention_matrix.mean():.6f}")
            logger.info(f"  Std:  {attention_matrix.std():.6f}")
            logger.info(f"  Diagonal mean: {np.diag(attention_matrix).mean():.6f}")
            logger.info(f"  Off-diagonal mean: {attention_matrix[~np.eye(30, dtype=bool)].mean():.6f}")
            logger.info(f"  Saved to: {save_path}.png/.pdf")
    
    def plot_class_separated_matrices(self, original_qfi: np.ndarray, attention_matrices: np.ndarray,
                                    labels: np.ndarray) -> None:
        """
        Plot class-separated average QFI matrices and attention matrices.
        
        Args:
            original_qfi: Original QFI matrices [N, 30, 30]
            attention_matrices: QFI-derived attention matrices [N, 30, 30]
        ...
        qcd_attention_avg = np.mean(attention_matrices[qcd_mask], axis=0)
        ttbar_attention_avg = np.mean(attention_matrices[ttbar_mask], axis=0)
        
        # Plot 4 average matrices - Original QFI
        ...
        self.plot_attention_matrix(
            qcd_attention_avg,
            "Average QFI-Derived Attention Matrix (QCD Jets)",
            "avg_attention_matrix_qcd",
            show_stats=True
        )
        
        self.plot_attention_matrix(
            ttbar_attention_avg,
            "Average QFI-Derived Attention Matrix (TTbar Jets)",
            "avg_attention_matrix_ttbar", 
            show_stats=True
        )
        
        # Compute and plot class differences
        original_diff = ttbar_original_avg - qcd_original_avg
        attention_diff = ttbar_attention_avg - qcd_attention_avg
        ...
        self.plot_attention_matrix(
            attention_diff,
            "Class Difference in QFI-Derived Attention (TTbar - QCD)",
            "class_difference_attention_matrix",
            show_stats=True
        )
        
        logger.success("Class-separated QFI and attention analysis completed")
...
def extract_attention_matrices_from_model(model: torch.nn.Module, batch) -> torch.Tensor:
    """
    Extract attention matrices from QuantumGATMessage layers in the model.
    
    Args:
        model: QFI GNN model with QuantumGATMessage layers
        batch: PyG batch object
        
    Returns:
        Attention matrices [batch_size, 30, 30]
    """
    from src.layers import QuantumGATMessage
    
    batch_size = int(batch.batch.max().item()) + 1
    device = batch.x.device
    
    # Find QuantumGATMessage layers in the model
    gat_layers = []
    for module in model.modules():
        if isinstance(module, QuantumGATMessage):
            gat_layers.append(module)
    
    if not gat_layers:
        logger.error("No QuantumGATMessage layers found in model!")
        return torch.zeros(batch_size, 30, 30, device=device)
    
    # Use the last GAT layer's attention weights
    last_gat_layer = gat_layers[-1]
    
    # Get attention matrix from the layer (assumes it has get_attention_matrix method)
    if hasattr(last_gat_layer, 'get_attention_matrix'):
        attention_matrix_single = last_gat_layer.get_attention_matrix(num_nodes=30, aggregation='mean')
        # Replicate for all samples in batch
        attention_matrices = attention_matrix_single.unsqueeze(0).repeat(batch_size, 1, 1)
    else:
        logger.warning("QuantumGATMessage layer doesn't have get_attention_matrix method")
        time.sleep(3)
        attention_matrices = torch.zeros(batch_size, 30, 30, device=device)
    
    return attention_matrices


def run_qfi_attention_inference_and_analysis(model: torch.nn.Module, loader: StreamingJetDataLoader,
                                           device: torch.device, max_batches: Optional[int] = None) -> Dict[str, Any]:
    """
    Run inference and collect QFI attention analysis.
    ...
    Returns:
        Dictionary with inference results and QFI/attention data
    """
    model.eval()
    ...
    logger.info("Running QFI attention inference and analysis...")
    
    batch_count = 0
    max_batches = max_batches or len(loader)
    
    with torch.no_grad():
        for batch in tqdm.tqdm(loader, desc="QFI Attention Inference", total=min(max_batches, len(loader))):
            ...
            # Extract original QFI and attention matrices
            try:
                # Original QFI reconstruction
                ...
                batch_attention_matrices = extract_attention_matrices_from_model(model, batch)
                attention_matrices.append(batch_attention_matrices.cpu().numpy())
                
            except Exception as e:
                logger.warning(f"Matrix extraction failed for batch {batch_count}: {e}")
    ...
    if original_qfi_matrices:
        ...
        attention_matrices = np.concatenate(attention_matrices, axis=0)
        logger.info(f"Collected matrices - QFI: {original_qfi_matrices.shape}, Attention: {attention_matrices.shape}")
    else:
        logger.error("No matrices were successfully extracted!")
...
def calculate_attention_analysis_metrics(results: Dict[str, Any]) -> Dict[str, float]:
    """Calculate attention matrix analysis metrics."""
    if 'attention_matrices' not in results or len(results['attention_matrices']) == 0:
        logger.warning("No attention matrices available for analysis")
        return {}
    
    attention_matrices = results['attention_matrices']
    
    # Vectorized statistics computation
    diagonal_values = np.array([np.diag(matrix) for matrix in attention_matrices])
    off_diagonal_mask = ~np.eye(30, dtype=bool)
    off_diagonal_values = np.array([matrix[off_diagonal_mask] for matrix in attention_matrices])
    
    metrics = {
        'attention_diag_mean': np.mean(diagonal_values),
        'attention_diag_std': np.std(diagonal_values),
        'attention_off_diag_mean': np.mean(off_diagonal_values),
        'attention_off_diag_std': np.std(off_diagonal_values),
        'attention_total_mean': np.mean(attention_matrices),
        'attention_total_std': np.std(attention_matrices),
        'attention_sparsity': np.mean(attention_matrices < 0.01)  # Fraction of near-zero weights
    }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze QFI-based Jet GNN with QFI-derived attention matrices",
    ...
    try:
        ...
        results_dir = os.path.join(args.experiment_dir, 'qfi_attention_analysis')
        Path(results_dir).mkdir(exist_ok=True)
        logger.info(f"QFI attention analysis results will be saved to: {results_dir}")
        ...
        # Run inference and QFI attention analysis
        results = run_qfi_attention_inference_and_analysis(
        ...
        # Calculate attention analysis metrics
        logger.info("Calculating attention analysis metrics...")
        attention_metrics = calculate_attention_analysis_metrics(results)
        
        # Combine all metrics
        all_metrics = {**classification_metrics, **attention_metrics}
        
        # Log results
        logger.info("=" * 60)
        logger.info("QFI GNN ATTENTION ANALYSIS RESULTS")
        logger.info("=" * 60)
        ...
        if attention_metrics:
            logger.info("QFI-Derived Attention Analysis:")
            logger.info(f"  Diagonal Mean: {attention_metrics['attention_diag_mean']:.6f}")
            logger.info(f"  Off-diagonal Mean: {attention_metrics['attention_off_diag_mean']:.6f}")
            logger.info(f"  Total Mean: {attention_metrics['attention_total_mean']:.6f}")
            logger.info(f"  Sparsity: {attention_metrics['attention_sparsity']:.4f}")
        logger.info("=" * 60)
        
        # Create QFI and attention visualizations
        if 'original_qfi' in results and len(results['original_qfi']) > 0:
            logger.info("Creating class-separated QFI and attention matrix visualizations...")
            plotter = QFIAttentionPlotter(results_dir)
            
            # Plot class-separated average matrices and differences
            plotter.plot_class_separated_matrices(
                results['original_qfi'], 
                results['attention_matrices'],
                results['labels']
            )
            ...
            plotter.plot_log_roc_curve(
                ...
                "QFI GNN with Attention"
            )
            
            logger.success("QFI attention visualizations completed")
        ...
        logger.success("QFI GNN attention analysis completed successfully!")
        ...
        logger.error(f"QFI GNN attention analysis failed: {e}")
